...
class Figure:public std::enable_shared_from_this<Figure>{
    ...
    //Storage of convex hull a.k.a. in this off set points of figure
    std::vector<std::shared_ptr<Point>> offsetPoints;
    //The center coordinates of this figure
    ...
    void convexHull();
    //Given convex hull finds this Figure's average center coordinates
    ...
    static void cleanConvex(std::vector<Point>& hull);
    //Moves 2 of convex hull points in order to get their off set by distance d ( which should be robot diameter )
    std::pair<Point, Point> moveLine(const Point& p1, const Point& p2, float d);
    //Clean ups convex hull, from unnecessary points in order to reduce amount of vias
    static void cleanUp(std::vector<Point>& hull);
