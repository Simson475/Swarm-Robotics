...
ln -s $(realpath experiment/libBotImplementations.so) ${new_folder}/libBotImplementations.so
...
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:./argos3-lib && ./argos3 -c ./trajectory.argos && {
    ...
    mv ${new_folder}/data.csv results_${2}/${new_folder}.csv

    # Remove all the data.
    rm -r $new_folder
} || {
    echo "The execution of ARGOS failed. See log-files for more details."; exit -1;
}